...
from tests_e2e.servers import (
    E2EServerStartTimeout,
    start_issue_tracker_api,
    start_mcp_server,
)
...
def test_start_mcp_server_raises_e2e_server_start_timeout_on_bad_command(monkeypatch, tmp_path):
    ...
    #
    # NOT a busy-port trick (unlike the analogous api-e2e test's directory-vs-file misuse, which
    # *is* still a fast local misuse): start_mcp_server's health check is a bare TCP connect
    # (see its docstring), so binding a blocker socket on the target port would make the health
    # check falsely report success against the blocker itself, never actually forcing (or
    # detecting) a real bind failure in the mcp-server subprocess — confirmed by direct
    # reproduction: uvicorn binding 0.0.0.0:<port> did not fail even with 127.0.0.1:<port>
    # already held open on this platform.
    with start_issue_tracker_api(tmp_path) as api_base_url:
