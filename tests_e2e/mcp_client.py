...
Verified directly against the installed mcp==2.1.1 SDK (inspect.getsource against
.venv/lib/python3.12/site-packages/mcp/client/streamable_http.py and mcp/client/session.py)
rather than assumed by analogy with other SDK versions:
  ...
  - `mcp.ClientSession(read_stream, write_stream)` is itself an async context manager; its
    ...
    schema-validation failure — see `mcp/server/mcpserver/server.py`'s
