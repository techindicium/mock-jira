...
- **Naming:** descriptive over clever; mirror JIRA's own vocabulary (issue, project, transition,
  status) so the mock reads as the thing it imitates.
...
- **Error handling:** fail at the HTTP boundary with a real JIRA-shaped error response, not a
...
- Reaching out to a real Atlassian endpoint for "realism."
