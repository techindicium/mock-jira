---
partial_schema: implement@1
affects: [issue-tracker-api, kanban-ui, mcp-server]
status: validated
risk_level: low
milestone: mvp
revision: 1
charter-revision: 4
created: 2026-09-04
updated: 2026-09-06
kind: behavioral
mode: cross-cutting
source-manifest:
  sha: "d92267d"
  files:
    - .context-index/constitution.md
    - CLAUDE.md
    - README.md
    - app/main.py
    - docker-compose.yml
    - docker/issue-tracker-api/Dockerfile
    - docker/mcp-server/Dockerfile
    - mcp_server/server.py
    - requirements.txt
    - tests/mcp_server/test_transport.py
    - tests/test_docker_deploy.py
    - tests/test_main_env.py
  computed-at: "2026-09-06T22:08:24.574Z"
drift_detected: true
---

# Live Spec: Docker packaging and run instructions

<!-- Live Spec for the deployment cross-cutting concern.
     This defines a specific behavioral contract that drives implementation and testing.
     Parent Charter: .context-index/specs/cross-cutting/deployment/charter.md -->

## Behavioral Contract

### Preconditions

- All three modules (`issue-tracker-api`, `kanban-ui`, `mcp-server`) are implemented and pass
  their own specs' acceptance criteria before packaging is meaningful to test.

### Behaviors

<!-- retired-behavior-ids: (none) -->

- **BEH-1** — **When** `docker compose build` is run, **then** it builds exactly two images:
  one for `issue-tracker-api` (which bundles `kanban-ui`'s built static assets during the build
  step, not at runtime), and one for `mcp-server`.
- **BEH-2** — **When** `docker compose up` is run, **then** `issue-tracker-api` starts first and
  becomes healthy before `mcp-server` starts, and `mcp-server` can reach it via `API_BASE_URL`.
- **BEH-3** — **When** the stack is running and `docker compose down` (without `-v`) is followed
  by `docker compose up` again, **then** the SQLite database file's contents persist unchanged —
  the named volume survives the down/up cycle.
- **BEH-4** — **When** a developer overrides `PORT` via environment variables, **then** compose
  remaps the exposed host ports with no code change in either module.
- **BEH-5** — **When** `docker compose logs` is run, **then** both containers' output appears in
  one combined stream.

### Postconditions

- Running `docker compose up` from a clean checkout (no prior containers, volumes, or images)
  brings up a fully working stack with no manual step beyond that one command.
- No port is exposed beyond localhost by default.

### Error Cases

| Condition | Expected Behavior | Error Code |
|-----------|-------------------|------------|
| The `DATABASE_PATH` volume mount is not writable | `issue-tracker-api` container fails startup with a clear error naming the path | `DEPLOY_VOLUME_NOT_WRITABLE` |
| `API_BASE_URL` is unset or unreachable for `mcp-server` | `mcp-server` container logs a clear connection error rather than crash-looping silently | `DEPLOY_UPSTREAM_UNREACHABLE` |

## Module Impact Map

| Module | Impact | Changes Required |
|--------|--------|-------------------|
| issue-tracker-api | high | Dockerfile; reads `DATABASE_PATH` and `PORT` from env; multi-stage build bundles kanban-ui's static assets |
| kanban-ui | low | No Dockerfile of its own — static build output copied into issue-tracker-api's image at build time |
| mcp-server | medium | Dockerfile; reads `API_BASE_URL` and `PORT` from env |

## Integration Points

1. `issue-tracker-api` ↔ compose: a basic liveness response backs the healthcheck; `DATABASE_PATH` binds to a named volume.
2. `mcp-server` ↔ `issue-tracker-api`: `depends_on` the API's healthcheck; reads `API_BASE_URL` to reach it.
3. `kanban-ui` ↔ `issue-tracker-api` (build-time only): static assets are copied into the API's image during `docker compose build`; there is no runtime integration point.

## System Constitution Reference

- **Principle:** "No inbound dependencies." — Applies because packaging must not introduce a
  dependency on another repo in the workspace. A public base image (e.g. `python:3.11-slim`)
  from a public registry is not an inbound repo dependency and is fine.
- **Principle:** "Fixture-backed, offline only." — Applies because no port is exposed beyond
  localhost by default, and the stack never reaches a real external network.

## Actionable Task Map

| Task | Description | Estimated Complexity |
|------|-------------|---------------------|
| `issue-tracker-api` Dockerfile | Multi-stage: build kanban-ui's static assets, then the Python runtime image; reads `DATABASE_PATH`/`PORT` from env | medium |
| `mcp-server` Dockerfile | Python runtime image; reads `API_BASE_URL`/`PORT` from env | small |
| `docker-compose.yml` | Wire both services, named volume for the SQLite file, healthcheck, `depends_on` | medium |
| Run instructions | Document `docker compose up`, exposed ports, and how to confirm the stack is healthy, in this repo's README/CLAUDE.md | small |

## Acceptance Criteria

- [x] `docker compose build` produces exactly two images (BEH-1)
- [x] `docker compose up` starts issue-tracker-api first, then mcp-server once healthy (BEH-2)
- [x] The SQLite file survives a `docker compose down`/`up` cycle (BEH-3)
- [x] `PORT` overrides remap host ports with no code change (BEH-4)
- [x] `docker compose logs` shows both containers' combined output (BEH-5)
- [x] No port is exposed beyond localhost by default
- [x] All quality gates pass (tests, lint)
- [x] No constitutional violations introduced
